...
hook.Add("TetrisInv.Hooks.PlayerLoadData", "TetrisInv.Hooks.PlayerLoadData.LoadInventory", function(ply)
    TETRIS_INV.FUNC.SQLQuery("SELECT * FROM tetrisinv_inventory WHERE userID = '" .. ply:TetrisInv():GetUserID() .. "';", function(data)
        ...
        for i = 1, #data do
            ...
            inventoryTable[i] = {
                itemData.entClass, {tonumber(itemData.transformX), tonumber(itemData.transformY), tonumber(itemData.transformW), tonumber(itemData.transformH), itemData.transformIsRotated == "1"},
...
function TETRIS_INV.PLAYERMETA:AddItem(class, itemData, itemSize)
    ...
    local key = table.insert(inventoryTable, {
        class, {itemX, itemY, itemW, itemH, false},
        itemData
    })
