...
net.Receive("TetrisInv.RequestMoveItem", function(len, ply)
    ...
    TETRIS_INV.FUNC.SQLQuery(string.format("UPDATE tetrisinv_inventory SET transformX=%d, transformY=%d, transformIsRotated=%d WHERE userID=%d AND transformX=%d AND transformY=%d", newX, newY, isRotated and 1 or 0, ply:TetrisInv():GetUserID(), oldX, oldY))
end)
